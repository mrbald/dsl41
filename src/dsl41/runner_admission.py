...
Not here yet: `expect` (stage S3 -- the gate below is where it goes), the
outbox and `executor_id` binding (S5), and epoch allocation (S6). `epoch`
is carried at 0 because ss6 ships it inert rather than break the wire
twice.
"""
...
class RequestCollision(EngineError):
