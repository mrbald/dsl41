...
Inlined verbatim into pages emitted by `dsl41 viz --html` so the report
renders offline (`file://`, zero network). Dev-time regeneration only —
users of the package never need node.
...
the script and by `tests/test_viz_html.py`:

- neither payload contains `</script` (they are inlined into `<script>`
  blocks without further escaping);
- the elk bundle starts with an attribution banner naming elkjs/EPL-2.0;
- `mermaid.min.js` matches its pinned sha256 (byte-exact from npm — the
  esbuild-built elk bundle is not byte-reproducible, so it gets a size
  floor instead);
