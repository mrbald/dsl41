...
def test_calendar_exports_lower_opaquely() -> None:
    ...
    assert set(catalog.calendars) == {
        "cal_exchange_holidays",
        "cal_last_workday",
        "cal_month_edges",
    }
    # the ext_calendar: spelling lowers to the same extended kind (Q9, DL-57)
    edges = catalog.calendars["cal_month_edges"]
    assert edges.kind == "extended"
    assert edges.conditions == ["FOMWORK", "EOMWORK"]
    ...
    # repeatable keys land in their lanes (DL-57)
    assert extended.attrs["cyccal"] == "cyc_quarter_close"
    assert "condition" not in extended.attrs
    assert extended.conditions == ["CWRK#L"]
    cycle = catalog.cycles["cyc_quarter_close"]
    assert isinstance(cycle, CycleIR)
    assert cycle.attrs == {"description": '"quarter close windows"'}
    assert cycle.periods == [("03/28/2026", "04/02/2026"), ("06/27/2026", "07/02/2026")]
...
@pytest.mark.parametrize(
    ("code", "expected"),
    [
        (5, False),  # listed in fail_codes -> FAILURE (the old fail-wins pin, now cited)
        (25, True),  # unlisted -> SUCCESS (also in success_codes, which is ignored)
        (0, True),  # unlisted -> SUCCESS even though success_codes omits it
        (2, True),  # unlisted -> SUCCESS; threshold ignored alongside fail_codes
        (99, True),  # unlisted, in NEITHER list -> SUCCESS ("any other exit code")
    ],
    ids=["listed-fails", "in-success", "zero-unlisted", "above-threshold", "in-neither"],
)
def test_dl33_exit_is_success_fail_codes_decide_alone(code: int, expected: bool) -> None:
    """Q7 RESOLVED (DL-58, KB 408778): a present fail_codes is the ONLY
    verdict source -- listed codes fail, 'any other exit code ... will be
    interpreted as a success'; success_codes and max_exit_success are
    ignored alongside it. (The superseded DL-33 pin routed unlisted codes
    to the threshold and consulted success_codes after a fail miss.)"""
    assert (
        exit_is_success(
            code,
            max_exit_success=2,
...
@pytest.mark.parametrize(
    ("code", "expected"),
    [
        (25, True),  # listed -> SUCCESS
        (0, False),  # success_codes REPLACES the default: 0 unlisted -> FAILURE
        (2, False),  # threshold ignored while success_codes present
    ],
