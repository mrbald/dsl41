...
def test_calendar_builders_populate_catalog_and_roundtrip() -> None:
    ...
    assert catalog.cycles["q1"].periods == [("03/28/2026", "04/02/2026")]
    assert catalog_hash(roundtrip(catalog)) == catalog_hash(catalog)


def test_multi_period_cycle_and_multi_condition_calendar_roundtrip() -> None:
    """DL-57: repeated start_date/end_date pairs and condition lines are
    legal vendor shapes (SEM-36/39); the lanes survive decompile."""
    c = CatalogBuilder()
    c.cycle("quarters", periods=[("01/01/2026", "01/05/2026"), ("04/01/2026", "04/05/2026")])
    c.extended_calendar("eom_or_fom", conditions=["EOMWORK", "FOMWORK"], non_workday="W")
    catalog = c.build()
    assert catalog.cycles["quarters"].periods == [
        ("01/01/2026", "01/05/2026"),
        ("04/01/2026", "04/05/2026"),
    ]
    assert catalog.calendars["eom_or_fom"].conditions == ["EOMWORK", "FOMWORK"]
    assert catalog_hash(roundtrip(catalog)) == catalog_hash(catalog)
