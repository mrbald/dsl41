...
@pytest.mark.parametrize(
    ("exit_code", "expected"),
    [(5, "Failed"), (25, "Success"), (0, "Success"), (31, "Success")],
    ids=["listed-fail-code", "in-success-range", "zero-unlisted", "outside-both"],
)
def test_m31_explicit_code_sets_share_the_autosys_verdict(exit_code: int, expected: str) -> None:
    ...
    data) -- the twin judges through ir.exit_is_success, so the cited Q7
    composition (DL-58, KB 408778: a present fail_codes decides alone,
    every unlisted code is Success) matches the AutoSys oracle by
    construction."""
    model = make_model(
        ["p"],
...
def test_pM07_mutex_overlap_queue_alignment_under_arm_and_wait() -> None:
    ...
    pre-DL-54 abandon reading, retired with Q3's resolution in DL-58,
    they diverged)."""
    text = (
        "insert_job: pm07_ja\njob_type: c\ncommand: x\nmachine: m1\n"
        'date_conditions: 1\ndays_of_week: all\nstart_times: "08:00"\ncondition: n(pm07_jb)\n\n'
