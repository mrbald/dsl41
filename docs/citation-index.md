...
| `L\d{3}` | Linter rule | One linter rule, L001–L020, with its stable code and severity. The house rule is a corpus fixture that trips the rule and one that does not; the lint suite names the exceptions it holds instead — L003 and L004 are defensive and structurally unreachable on real input, and L017 is registered but quiet because the corpus defines every machine it references. | `docs/ir-design.md` §9 |
